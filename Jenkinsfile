pipeline {
    ...
    stages {
        ...
        stage('Deploy backend'){
            steps {
                dir('future-traffic') {
                    sh "docker build -t esp22-backend ."
                    sh "docker tag esp22-backend 192.168.160.99:5000/esp22-backend"
                    sh "docker push 192.168.160.99:5000/esp22-backend"
                }
            }
        }
        ...
        stage('Deploy runtime'){
            steps{
                sshagent(credentials: ['future-traffic-runtime']){
                    ...
                    sh "ssh -o 'StrictHostKeyChecking=no' -l esp22 192.168.160.103 docker run -d -p 6080:8888 --name esp22-backend 192.168.160.99:5000/esp22-backend"
